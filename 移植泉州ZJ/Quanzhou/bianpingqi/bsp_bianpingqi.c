...
//		rULCON3 =0x2B; 								//0x2b=00 101 0 11  普通 偶校验（even） 1停止位 8数据位
	...
	GPIO_SetBits(RS485_REDE_PORT,RS485_REDE_PIN);//Write，使用MAX485芯片需要改变位
...
//		while((!(rUTRSTAT3 & 0x4)) && (waitTime<=ErrorLoop)){
	   while(( waitTime<=ErrorLoop)&&(USART_GetFlagStatus(RS485_USARTx,USART_FLAG_TC)!=SET)){
			for (i=0;i<50;i++){
...
//		bianpingqi_RTU_WriteWord(bianpingqi_write_fun_num,0x1001,4);0100=4	//反转启动
		bianpingqi_jog_button=0;
	}
	if (bianpingqi_run_button==0&&bianpingqi_jog_status==0 && bianpingqi_previous_run_status==1)
	{
		bianpingqi_stop_sub();
//		bianpingqi_RTU_WriteWord(bianpingqi_write_fun_num,0x1001,(int)(1<<7));0100 0000
